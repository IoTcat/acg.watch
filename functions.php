...
function getVideoAll(){
    ...
    $json_string = file_get_contents('../cache/video.json');
    return json_decode($json_string);
}

function getVideoClass(){
    return array_keys(getVideoClass());
}

function getSeriesByClass($class){
    $obj = getVideoAll();
    return array_keys($obj[$class]);
}

function getClassBySeries($series){
    $obj = getVideoAll();
    forEach($obj as $class => $val){
      forEach($val as $subseries => $subval){
        if($subseries == $series){
            return $class;
        }
      }
...
function getVideoByUrl($url){
    ...
    if(substr($url, 0, 34) != "https://onedrive.yimian.xyz/video/"){
        return array(
            "url" => $url,
            "series" => "Unknown",
            "name" => "Unknown"
    ...
    $arr = explode("/", substr($url, 34));
    $class = getClassBySeries($arr[0]);
var_dump((Array)($obj->$class)[$arr[0]]);
    $o = array(
        "class" => $class,
        "series" => $obj->$class->$arr[0]['series'],
        "url" => $url
    );

    $vid = getVidByUrl($url,  $obj[$class][$arr[0]]['video']);

    $array_push($o, $obj[$class][$arr[0]]['video'][$vid]);
